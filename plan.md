...
**작업 항목**:
- [ ] Goal 공유 기반 다중 에이전트 협업 플로우 구현 (`goals` 테이블 활용)
- [ ] 페르소나 에이전트 System Prompt 템플릿 (산업군별 10개 이상)
- [ ] 에이전트 간 메시지 전달 프로토콜 구현
- [ ] 무한 루프 방지 (최대 반복 횟수 제한, 타임아웃 설정)
...
**작업 항목**:
- [ ] 포트폴리오 기획서 작성 에디터 UI
- [ ] 독창성 점수 게이지 바 (실시간 업데이트)
- [ ] 다중 에이전트 인터뷰 채팅 UI (페르소나별 아바타 표시)
- [ ] 단계별 진행 상황 트래커 (인터뷰 → 기획서 → 보안검토 → 독창성 인증)
- [ ] 관리자 GUI — 포트폴리오 설정 페이지
  - 유사도 기준 슬라이더 (기본 60%)
