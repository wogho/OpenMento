...
| 수강생 데모 | [openmento.cloud/auth/student](https://openmento.cloud/auth/student-login) |
...
시스템 설정이 완료된 후, 강사가 AI 튜터를 개설하고 수강생에게 연결하기까지의 전체 흐름입니다.

### 0단계 — 에이전트 API 키 확인

**보안 키 관리** 메뉴에서 사용할 AI 에이전트의 API 키가 등록되어 있는지 확인합니다.  
키가 없으면 에이전트가 동작하지 않습니다.

> 지원 에이전트: OpenAI (GPT-4o), Anthropic (Claude), Google Gemini, Gemini CLI, HTTP Webhook

---

### 1단계 — 스킬(Skill) 작성 및 에이전트 지정

**스킬 관리** 메뉴에서 AI의 행동 지침이 담긴 스킬을 작성합니다.

- 기본 제공 스킬(오케스트레이터, AI 튜터, EWS 모니터 등)을 그대로 사용하거나 직접 작성할 수 있습니다.
- 스킬 작성 후, 해당 스킬을 실행할 **에이전트(모델)를 지정**합니다.
- 스킬에는 참고할 **교재(문서) 태그**를 함께 연결하면 RAG 기반 맞춤 응답이 가능합니다.
...
| AI 강사 | 커리큘럼 기반 코드 리뷰 |
| AI 튜터 | 소크라테스식 질문 응답 |
| EWS 모니터 | 수강생 이탈 위험 감지 |
| 멘탈케어 | 공감적 어조 상담 |
| 포트폴리오 심사 | 유사도 분석 및 피드백 |

---

### 2단계 — 수강생 추가 및 강사 지정

**학생 관리** 메뉴에서 수강생을 추가하거나, 기존 수강생 프로필에서 담당 강사를 지정합니다.

- 신규 수강생: **초대 링크**를 생성해 수강생이 직접 계정을 만들 수 있습니다.
- 기존 수강생: 프로필 편집에서 담당 강사 및 과목을 연결합니다.

---

### 3단계 — 과목 생성 및 수강생·스킬 연결

**과목 관리** 메뉴에서 과목을 생성하고 다음을 설정합니다.
...
**학생 관리 → 대상 수강생 선택 → 스킬 매핑**에서 수강생 개인에게 맞는 스킬을 할당합니다.

- 수강생은 로그인 후 채팅 목록에서 과목별 AI 튜터와 바로 대화를 시작할 수 있습니다.
- 강사가 스킬을 변경하면 다음 대화부터 즉시 반영됩니다.

<br/>

## 수강생 사용 가이드

강사가 과목과 스킬을 설정한 후, 수강생이 AI 튜터와 상호작용하는 전체 흐름입니다.

### 1단계 — 과목 입장

로그인 후 **채팅 목록** 화면에서 강사가 지정한 과목을 선택해 입장합니다.  
각 항목에는 담당 강사명, 과목명, 마지막 대화 미리보기가 표시됩니다.

---

### 2단계 — 에이전트와 대화

과목 채팅창에서 AI 에이전트와 자유롭게 대화할 수 있습니다.

- 에이전트는 강사가 설정한 스킬을 기반으로 응답합니다.
- 교재에 태깅된 문서가 있으면 해당 내용을 참고한 답변을 제공합니다.
- 정답을 바로 알려주는 대신, 스킬 설정에 따라 소크라테스식 문답으로 사고를 유도할 수 있습니다.

---

### 3단계 — AI 간 토론 기능 (선택)

수강생에게 **에이전트가 2개 이상 배정**된 경우, 채팅 상단의 토론 모드 버튼으로 AI 간 토론 기능을 활성화할 수 있습니다.

- 서로 다른 관점을 가진 에이전트들이 주제에 대해 논의하며 수강생의 비판적 사고를 자극합니다.
- 토론 종료 후 수강생은 자신의 최종 의견을 정리해 제출할 수 있습니다.

---

### 4단계 — 과목 채팅 메뉴 활용

채팅창 우측 메뉴에서 다음 기능을 이용할 수 있습니다.

| 기능 | 설명 |
|------|------|
| **강사 호출** | 담당 강사와 1:1 채팅을 시작합니다 |
| **과제 보기** | 현재 과목에 등록된 과제 목록을 확인합니다 |
| **에이전트 활성화/비활성화** | 배정된 에이전트 중 현재 대화할 에이전트를 선택합니다 |
| **자율 발화** | 에이전트가 주기적으로 학습 상황을 확인하는 자동 메시지를 활성화합니다 |

> **자율 발화 사용 전 확인**: 수강생에게 두개 이상의 에이전트가 존재해야하며,관리자가 **에이전트 설정**에서 아래 두 항목을 모두 활성화해야 사용할 수 있습니다.
> 1. **주기적 하트비트 실행** — 에이전트가 일정 주기마다 자동으로 깨어나도록 설정
> 2. **자율 발화 활성화 (프로액티브)** — 에이전트가 수강생에게 먼저 메시지를 보내는 기능 허용
