...
<p align="center">
  ...
  <a href="https://github.com/wogho/EduClip"><strong>GitHub</strong></a>
</p>

<p align="center">
  <a href="https://github.com/wogho/EduClip/blob/main/LICENSE"><img src="https://img.shields.io/badge/license-MIT-blue" alt="MIT License" /></a>
</p>
...
EduClip은 AI 에이전트 팀을 조직하여 교육 기관의 운영 전반을 자동화하는 Node.js 서버와 React 대시보드입니다. 다양한 에이전트를 배치하고, 교육 목표를 할당하며, 단일 대시보드에서 에이전트의 작업과 비용을 추적할 수 있습니다.
...
EduClip은 단순한 온라인 교육 플랫폼(LMS) 구축에 국한되지 않습니다.  
...
| 아이디어가 생기면 직접 AI를 열고, 탭을 유지하며, 진행을 일일이 모니터링해야 합니다.                   | EduClip에 작업을 추가하면 담당 에이전트가 완료 시까지 처리하고 상위 에이전트가 결과를 검토합니다.        |

<br/>

## EduClip이 특별한 이유

EduClip은 까다로운 오케스트레이션 세부 사항을 올바르게 처리합니다.
...
| **워크플로우 빌더가 아닙니다** | 드래그 앤 드롭 파이프라인이 없습니다. EduClip은 교육 기관을 모델링합니다 — 조직도, 목표, 예산, 거버넌스 포함. |
| **프롬프트 관리자가 아닙니다** | 에이전트는 자체 프롬프트, 모델, 런타임을 가져옵니다. EduClip은 에이전트들이 속한 조직을 관리합니다.    |
| **단일 에이전트 도구가 아닙니다** | 팀을 위한 플랫폼입니다. 에이전트가 한 명이라면 EduClip이 필요 없을 수 있습니다. 스무 명이라면 반드시 필요합니다. |
...
```bash
git clone https://github.com/wogho/EduClip.git
cd EduClip/educlip
pnpm install
pnpm dev
```
...
**에이전트가 지속적으로 실행됩니까?**  
기본적으로 에이전트는 예약된 하트비트와 이벤트 기반 트리거(작업 할당, 멘션 등)로 실행됩니다. 연속 에이전트도 연결할 수 있으며 EduClip이 이를 조율합니다.
...
- [GitHub Issues](https://github.com/wogho/EduClip/issues) — 버그 리포트 및 기능 요청
- [GitHub Discussions](https://github.com/wogho/EduClip/discussions) — 아이디어 및 RFC

<br/>

## 라이선스

MIT &copy; 2026 EduClip
