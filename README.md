...
AI 시대의 교육 현장에서 가장 심각한 문제 중 하나는 **AI 도구를 이용한 복붙·양산형 결과물의 범람**입니다.  
OpenMento는 이를 구조적으로 차단합니다.

| 기능 | 설명 |
| ---- | ---- |
| **제출물 유사도 분석** | 수강생 제출물을 벡터 임베딩으로 변환하여 동기 기수·과거 제출물 전체와 의미론적 유사도를 실시간 비교합니다. 텍스트가 달라도 내용이 같으면 탐지합니다. |
| **학습 과정 추적** | 최종 결과물만이 아닌 작성 과정(단계별 저장 이력, 수정 패턴 등)을 분석하여 학습 주도성을 평가합니다. |
| **개인화 검증 문항** | 제출 후 AI 튜터 에이전트가 결과물 내용을 바탕으로 수강생에게 구술 형식의 후속 질문을 자동 생성합니다. 내용을 이해한 수강생만 통과할 수 있습니다. |
| **레퍼런스 다양성 평가** | 참고 자료의 출처·범위·시점 분포를 분석하여 특정 소스에 과의존한 결과물을 식별합니다. |
| **에이전트 감사 로그** | 에이전트의 모든 평가 판단과 피드백 생성 근거를 로그로 기록하여 교강사가 언제든 검토하고 이의를 제기할 수 있습니다. |

> 이 기능들은 수강생을 의심하는 것이 아닙니다.  
> **실력이 쌓이는 학습 환경을 보장**하고, 진짜 실력으로 완성된 포트폴리오에 정당한 가치를 부여하기 위함입니다.
...
<td align="center">
<h3>모바일 지원</h3>
어디서든 스마트폰으로 교육 기관 운영 현황을 모니터링하고 관리할 수 있습니다.
</td>
</tr>
</table>
...
OpenMento는 까다로운 오케스트레이션 세부 사항을 올바르게 처리합니다.

|                              |                                                                                                            |
| ---------------------------- | ---------------------------------------------------------------------------------------------------------- |
| **원자적 실행**              | 작업 체크아웃과 예산 적용이 원자적으로 처리되어 중복 작업이나 비용 초과가 발생하지 않습니다.               |
| **지속적 에이전트 상태**     | 에이전트는 하트비트 사이에도 동일한 작업 컨텍스트를 유지하며 처음부터 재시작하지 않습니다.                 |
| **런타임 스킬 주입**         | 에이전트가 플랫폼 워크플로우와 교육 프로젝트 컨텍스트를 런타임에 학습할 수 있어 재학습이 필요 없습니다.    |
| **롤백 포함 거버넌스**       | 승인 게이트가 적용되고 설정 변경 이력이 관리되어 잘못된 변경을 안전하게 되돌릴 수 있습니다.                |
| **목표 인식 실행**           | 작업이 전체 목표 계층을 포함하여 에이전트가 제목만이 아닌 수행 이유를 일관되게 인식합니다.                  |
| **기관 템플릿 이식성**       | 기관, 에이전트, 스킬을 시크릿 제거 및 충돌 처리와 함께 내보내고 가져올 수 있습니다.                       |
| **완전한 멀티 기관 격리**     | 모든 엔티티가 기관 범위로 분리되어 단일 배포 환경에서 여러 기관을 독립된 데이터와 감사 추적으로 운영합니다. |
